...
namespace TerrariaHackGUI
{
    public partial class TerrariaEditor : Form
    {
        ...
        public TerrariaEditor()
        {
            ...
            BuffBox.Nodes.Add("Buffs");
            
            foreach (var i in IDNameDictionary.Buffs)
            {
                BuffList.Items.Add(i.Value);
            }
        ...
        public void PlayerLoaded()
        {
            ...
            LifeBox.Value = player.Life;
            MaxLifeBox.Value = player.MaxLife;
            ManaBox.Value = player.Mana;
            MaxManaBox.Value = player.MaxMana;

            HairStyleBox.Value = player.Hair;
            HairDyeBox.SelectedIndex = HairDyeBox.FindString(IDNameDictionary.HairDye[player.HairDye]);

            ExtraAccessoryBox.Checked = player.ExtraAccessory;

            AnglerQuestsBox.Value = player.AnglerQuestsFinished;

            TaxMoneyBox.Value = player.TaxMoney;
            ...
            BuffBox.Nodes[0].Nodes.Clear();
            foreach (Buff b in player.Buffs)
            {
                TreeNode buff = new TreeNode();
                buff.Text = b.ToString();
                buff.Tag = b;
                BuffBox.Nodes[0].Nodes.Add(buff);
            }
        }
        ...
        private void SaveCharacterButton_Click(object sender, EventArgs e)
        {
            if (CurrentPath != null)
            {
                ...
                player.Life = (int)LifeBox.Value;
                player.MaxLife = (int)MaxLifeBox.Value;
                player.Mana = (int)ManaBox.Value;
                player.MaxMana = (int)ManaBox.Value;

                player.Hair = (int)HairStyleBox.Value;
                player.HairDye = (byte)(HairDyeBox.SelectedIndex);

                player.ExtraAccessory = ExtraAccessoryBox.Checked;

                player.AnglerQuestsFinished = (int)AnglerQuestsBox.Value;

                player.TaxMoney = (int)TaxMoneyBox.Value;

                for (int i = 0; i < 22; i++)
                {
                    player.Buffs[i] = BuffBox.Nodes[0].Nodes[i].Tag as Buff;
                }
        ...
        {
            if (CurrentPath != null && Inventory.SelectedNode != null)
        ...
        private void BuffSaveButton_Click(object sender, EventArgs e)
        {
            if (CurrentPath != null && BuffBox.SelectedNode != null)
                if (BuffBox.SelectedNode.Text != "Buffs"
                 && BuffBox.SelectedNode.Tag != null)
                {
                    BuffBox.SelectedNode.Tag = 
                        new Buff
                        {
                            BuffTime = (int)BuffTimeChooser.Value,
                            BuffType = IDNameDictionary.Buffs.First(x => x.Value == BuffList.SelectedItem.ToString()).Key
                        };
                    BuffBox.SelectedNode.Text = BuffBox.SelectedNode.Tag.ToString();
                }
        }
    }
}
